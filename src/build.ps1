...
$OutputPath = "$BuildFolder\output\UniversalDashboard.DudSearch"
...
Copy-Item $BuildFolder\UniversalDashboard.DudSearch.psm1 $OutputPath
Copy-Item $BuildFolder\Scripts $OutputPath\Scripts -Recurse -Force

$Version = "1.0.0"

$manifestParameters = @{
    Path              = "$OutputPath\UniversalDashboard.DudSearch.psd1"
    Author            = "Francis Mercier"
    CompanyName       = "NZT"
    Copyright         = "2019 NZT"
    RootModule        = "UniversalDashboard.DudSearch.psm1"
    Description       = ""
    ModuleVersion     = $Version
    Tags              = @("universaldashboard", "DudDashboard")
    ReleaseNotes      = "."
    FunctionsToExport = @(
        "New-DudSearch"
    )
